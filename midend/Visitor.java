...
public class Visitor {
    ...
    * */
    ...
    public void visitForStmt(StmtNode stmtNode) {
        ...
        // TODO 修改了顺序，这里的影响未知
        // forstmt1 - > cond -> body -> change -> cond
        //curFunction.getBasicBlocks().remove(changeBlock);
        //curFunction.getBasicBlocks().add(changeBlock);
    }
    ...
    public void visitGetintStmt(StmtNode stmtNode) {
        ...
        /*calledby and calling*/
        curFunction.addCallingFunction(function);
        assert function != null;
        function.addCalledByFunction(curFunction);
    ...
    public void visitPrintfStmt(StmtNode stmtNode) {
        ...
        for (int i = 0; i < formatString.length(); i++) {
               if (formatString.charAt(i) == '%' && formatString.charAt(i + 1) == 'd') {
                   ...
                   /*
                   int j = i;
                   while (formatString.charAt(j) != '%') {
                       j++;
                       if (j >= formatString.length()) {
                           break;
                       }
                   }
                   String str = formatString.substring(i,j);
                   GlobalVar globalString = IRBuilder.buildGlobalString(str);
                   pushSymbol(globalString.getValueName(),globalString); // push into symbol table
                   IRModule.getInstance().addGlobalVar(globalString);
                   ArrayList<Value> indices = new ArrayList<>();
                   indices.add(new ConstInteger(32,0));
                   indices.add(new ConstInteger(32,0));

                   Value gep = IRBuilder.buildGetElementPtr(globalString,indices,curBasicBlock);

                   String functionName = "putstr";
                   Function function = (Function) find(functionName);
                   ArrayList<ValueType> parametersType  = new ArrayList<>();
                   parametersType.add(new PointerType(new IntegerType(8)));
                   FunctionType functionType = new FunctionType(new VoidType(),parametersType);
                   ...
                   IRBuilder.buildCall(function,functionType,curBasicBlock,arguments);
                   i = j -1;
                   */
    ...
    public void visitEqExp(EqExpNode eqExpNode , String formerOperation) {
        ...
        if (eqExpNode1 == null && formerOperation == null && !(curValue.getValueType() instanceof IntegerType && ((IntegerType)curValue.getValueType()).getBits() == 1)) {
